...
export class bankAccountService{
    ...
    async add(user: User){
       ...
       await newAccount.populate("category bankAccount");
       return newAccount;
    }

    async getById(idAcc: string){
        const prova = await Account.findOne({_id: idAcc}).populate("user");
        //const prova = await AccountModel.find();
        return prova;
    }
